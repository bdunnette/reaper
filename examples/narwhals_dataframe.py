...
async def run_async_example():
    ...
    async with AsyncRealtorClient() as client:
        try:
            print(
                "Searching properties in Austin, TX directly as a DataFrame (async)..."
            )
            df = await client.search_properties_dataframe(
                location="Austin, TX", price_max=800000, max_results=5
            )
    ...
    with RealtorClient() as client:
        try:
            ...
            search_res = client.search_properties(
                location="Austin, TX", price_max=800000, beds_min=3, limit=10
            )
            ...
            try:
                ...
                print(
                    filtered_df.select(
                        ["property_id", "list_price", "beds", "sqft", "address_line"]
                    )
                )
                ...
                print(
                    direct_df.select(
                        ["property_id", "list_price", "beds", "address_line"]
                    )
                )
                ...
                print(f"\n[Environment Info]: {e}")
                print(
                    "Tip: Run `pip install polars` or `pip install pandas` to test the dataframe conversion."
                )
